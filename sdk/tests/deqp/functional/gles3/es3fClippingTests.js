...
/**
 * Generate vertices close to clipping volume limits,
 * both inside and outside of the clipping volume.
 * @param {number} count
 * @return {Array<number>}
 */
es3fClippingTests.generateVertices = function(count) {
    var rnd = new deRandom.Random(0xabcd);
    var positions = [];
    var i = 0;
    for (i = 0; i < count / 6; i++) {
        var x = rnd.getFloat(-1.1, 1.1);
        var y = rnd.getFloat(-1.1, -0.9);
        if (i % 2 == 0)
            y = -y;
        var z = rnd.getFloat(-1.1, -0.9);
        if (i % 4 < 2)
            z = -z;
        var v = [x, y, z, 1];
        positions.push(v);
        positions.push(deMath.swizzle(v, [0, 2, 1, 3]));
        positions.push(deMath.swizzle(v, [1, 0, 2, 3]));
        positions.push(deMath.swizzle(v, [1, 2, 0, 3]));
        positions.push(deMath.swizzle(v, [2, 0, 1, 3]));
        positions.push(deMath.swizzle(v, [2, 1, 0, 3]));
    }
    return [].concat.apply([], rnd.shuffle(positions));
};
 ...
 * @param {string} desc
 */
es3fClippingTests.LinesCase = function(name, desc, viewport) {
    es3fFboTestCase.FboTestCase.call(this, name, desc);
    this.m_viewport = viewport;
};
...
es3fClippingTests.LinesCase.prototype.render = function(dst) {
        ...
        var positions = es3fClippingTests.generateVertices(64);
...
es3fClippingTests.PointsCase.prototype.render = function(dst) {
        ...
        var program = ctx.createProgram(shader);
        shader.setColor(ctx, program, [1, 1, 1, 1]);
        ...
        var positions = es3fClippingTests.generateVertices(256);
...
es3fClippingTests.ClippingTests.prototype.init = function() {
    ...
    var viewports = [
        { name: 'full_viewport', v: [0, 0, width, height] },
    ...
    for (var i = 0; i < viewports.length; i++) {
        ...
        this.addChild(new es3fClippingTests.LinesCase('lines_' + vName, 'lines', v));
        this.addChild(new es3fClippingTests.PointsCase('small_points_' + vName, 'points', v, 1));
        if (pointSizeRange[1] >= 5)
            this.addChild(new es3fClippingTests.PointsCase('wide_points_' + vName, 'points', v, 5));
    }
